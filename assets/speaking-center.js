(() => {
  ...
  function renderFeedback(attempt){
    ...
    $$('[data-speaking-language]').forEach(b=>b.onclick=()=>{const x=a.language[Number(b.dataset.speakingLanguage)],category=attempt.part==='Part 1'?'Part 1':(/person|people|friend|family|teacher/i.test(attempt.question)?'人物类':/place|city|building|country/i.test(attempt.question)?'地点类':/object|thing|gift|book/i.test(attempt.question)?'物品类':'Part 3 观点'),ok=window.addIELTSSpeakingCard?.({category,front:x,meaning:'来自口语练习，中文待补充',example:attempt.transcript,note:attempt.title});alert(ok?'已加入“口语专用记忆卡”。':'这条表达已存在。')});$('#speakingFeedback').scrollIntoView({behavior:'smooth',block:'start'});
  }
