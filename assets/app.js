...
Promise.all(['data/plan.json','data/topics.json','data/part1.json','data/progress.json','data/vocabulary.json','data/story-materials.json'].map(x=>fetch(`${x}?v=5`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`${x} (${r.status})`);return r.json()}))).then(([plan,topics,part1,progress,vocabulary,storyMaterials])=>{
  ...
  const due=c=>!memoryState[c.id]?.archived&&(memoryState[c.id]?.due||0)<=Date.now();
  function saveMemory(){localStorage.setItem(STORAGE,JSON.stringify(memoryState))}
  function updateMemoryArchived(){const n=vocabulary.filter(c=>memoryState[c.id]?.archived).length;$('#memoryArchivedCount').textContent=n}
  function buildQueue(){queue=vocabulary.filter(c=>inDeck(c)&&due(c));if(!queue.length)queue=vocabulary.filter(c=>inDeck(c)&&!memoryState[c.id]?.archived).sort((a,b)=>(memoryState[a.id]?.due||0)-(memoryState[b.id]?.due||0)).slice(0,5);initial=queue.length;reviewed=0;updateMemoryArchived();showNext()}
  ...
  $('#archiveMemory').addEventListener('click',()=>{if(!current)return;memoryState[current.id]={...(memoryState[current.id]||{}),archived:true,archivedAt:Date.now()};saveMemory();reviewed++;updateMemoryArchived();showNext()});
  $('#restoreMemory').addEventListener('click',()=>{const ids=vocabulary.filter(c=>memoryState[c.id]?.archived).map(c=>c.id);if(!ids.length){alert('没有已移出的词汇。');return}if(!confirm(`恢复 ${ids.length} 个已移出的词汇吗？`))return;ids.forEach(id=>{memoryState[id].archived=false;memoryState[id].due=0});saveMemory();buildQueue()});
  buildQueue();
