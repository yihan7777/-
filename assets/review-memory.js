(() => {
  ...
  function setupListeningIntensive(){
    ...
    function extractStaticTranscript(html){try{const doc=new DOMParser().parseFromString(String(html||''),'text/html');return [...doc.querySelectorAll('[data-time],[data-start],.transcript-line')].map((node,index)=>{const raw=node.dataset.time||node.dataset.start||'',time=cueSeconds(raw),text=node.textContent.trim();return time===null||!text?null:{index,time,label:raw,text}}).filter(Boolean)}catch(_){return[]}}
    function renderSyncTranscript(){syncCues=parseSyncTranscript($('#intensiveTranscript').value);if(!syncCues.length&&current?.record?.html)syncCues=extractStaticTranscript(current.record.html);$('#intensiveSyncList').innerHTML=syncCues.length?syncCues.map((cue,i)=>'<button data-sync-cue="'+i+'"><time>'+esc(cue.label)+'</time><span>'+esc(cue.text)+'</span></button>').join(''):'<p>请按“[00:12] 原文句子”格式填写；生成后点击任意一句即可从对应时间播放。</p>';$('[data-sync-cue]').forEach(b=>b.onclick=()=>{const cue=syncCues[Number(b.dataset.syncCue)],audio=$('#intensiveAudio');if(!cue||!audio.src)return alert('当前设备没有这篇音频。');audio.currentTime=cue.time;audio.play()})}
