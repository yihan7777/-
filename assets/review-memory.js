(() => {
  ...
  function setupSpeakingMemory(){
    ...
    const categoryPart=x=>/^P1\b|^Part 1\b/i.test(x)?'P1':/^P2\b|^Part 2\b/i.test(x)?'P2':/^P3\b|^Part 3\b/i.test(x)?'P3':'其他';
  ...
  function setupListeningIntensive(){
    ...
    let syncCues=[];
    function cueSeconds(value){const m=String(value||'').trim().match(/^(?:(\d+):)?(\d{1,2})(?::(\d{1,2}(?:\.\d+)?))?$/);if(!m)return null;return m[3]!==undefined?Number(m[1]||0)*3600+Number(m[2])*60+Number(m[3]):Number(m[1]||0)*60+Number(m[2])}
    function parseSyncTranscript(text){return String(text||'').split(/\r?\n/).map((line,index)=>{const m=line.trim().match(/^\[?(\d{1,2}:\d{2}(?::\d{2})?)\]?\s*[-–—]?\s*(.+)$/);if(!m)return null;const time=cueSeconds(m[1]);return time===null?null:{index,time,label:m[1],text:m[2].trim()}}).filter(Boolean).sort((a,b)=>a.time-b.time)}
    function formatCueTime(seconds){const value=Math.max(0,Math.round(Number(seconds)||0)),m=Math.floor(value/60),s=value%60;return String(m).padStart(2,'0')+':'+String(s).padStart(2,'0')}
    function decodeScriptString(raw, quote){try{return quote==='"'?JSON.parse('"'+raw+'"'):raw.replace(/\\'/g,"'").replace(/\\n/g,'\\n').replace(/\\r/g,'').replace(/\\t/g,' ').replace(/\\\//g,'/').replace(/\\\\/g,'\\')}catch(_){return raw}}
    function plainTranscript(value){const d=document.createElement('div');d.innerHTML=String(value||'');return(d.textContent||'').replace(/\\s+/g,' ').trim()}
    function extractScriptTranscript(html){const source=String(html||''),keys=['transcriptLines','transcript','tapescript','audioScript'];for(const key of keys){const match=new RegExp('(?:["\\\\\']?'+key+'["\\\\\']?)\\\\s*:\\s*\\[').exec(source);if(!match)continue;let pos=match.index+match[0].length-1,depth=0,quote='',escaped=false,end=-1;for(let i=pos;i<source.length;i++){const ch=source[i];if(quote){if(escaped)escaped=false;else if(ch==='\\\\')escaped=true;else if(ch===quote)quote='';continue}if(ch==='"'||ch==="'"||ch==='\`'){quote=ch;continue}if(ch==='[')depth++;else if(ch===']'&&--depth===0){end=i+1;break}}if(end<0)continue;const chunk=source.slice(pos,end);try{const rows=JSON.parse(chunk);if(Array.isArray(rows)){const cues=rows.map((row,index)=>{const rawTime=row?.time??row?.start??row?.begin??row?.seconds,time=cueSeconds(rawTime),text=plainTranscript(row?.html??row?.text??row?.transcript??row?.line);return !text?null:{index,time:time===null?null:time,label:time===null?'':formatCueTime(time),text}}).filter(Boolean);if(cues.length)return cues}}catch(_){}const cues=[];const item=/\\{[\\s\\S]*?\\}/g;let found;while((found=item.exec(chunk))){const block=found[0],textMatch=/(?:html|text|transcript|line)\\s*:\\s*(["'])([\\s\\S]*?)\\1/i.exec(block);if(!textMatch)continue;const timeMatch=/(?:time|start|begin|seconds)\\s*:\\s*(["']?)([0-9:.]+)\\1/i.exec(block),time=timeMatch?cueSeconds(timeMatch[2]):null,text=plainTranscript(decodeScriptString(textMatch[2],textMatch[1]));if(text)cues.push({index:cues.length,time,label:time===null?'':formatCueTime(time),text})}if(cues.length)return cues}return[]}
    function extractStaticTranscript(html){try{const doc=new DOMParser().parseFromString(String(html||''),'text/html');const dom=[...doc.querySelectorAll('[data-time],[data-start],.transcript-line,.tapescript-line')].map((node,index)=>{const raw=node.dataset.time||node.dataset.start||node.getAttribute('data-begin')||'',time=cueSeconds(raw),text=node.textContent.replace(/\\s+/g,' ').trim();return time===null||!text?null:{index,time,label:formatCueTime(time),text}}).filter(Boolean);if(dom.length)return dom;return extractScriptTranscript(html).filter(x=>x.time!==null)}catch(_){return extractScriptTranscript(html).filter(x=>x.time!==null)}}
    function extractTranscriptText(html){try{const scripted=extractScriptTranscript(html);if(scripted.length)return scripted.map(x=>x.text).join('\\n');const doc=new DOMParser().parseFromString(String(html||''),'text/html');doc.querySelectorAll('style,noscript,nav').forEach(n=>n.remove());const direct=[...doc.querySelectorAll('[data-transcript],#transcript,#tapescript,.transcript,.tapescript,.tape-script,.audio-script,.script-text,.listening-script')].map(n=>n.innerText||n.textContent||'').find(t=>String(t).replace(/\\s+/g,' ').trim().length>80);if(direct)return direct;doc.querySelectorAll('script').forEach(n=>n.remove());const heading=[...doc.querySelectorAll('h1,h2,h3,h4,h5,b,strong,summary,button')].find(n=>/^(原文|听力原文|录音原文|transcript|tapescript|audio script|script)$/i.test((n.textContent||'').replace(/[：:]/g,'').trim()));if(!heading)return'';let text='',node=heading.nextElementSibling;while(node&&!/^H[1-5]$/.test(node.tagName)&&text.length<24000){text+='\\n'+(node.innerText||node.textContent||'');node=node.nextElementSibling}return text}catch(_){return''}}
        function splitTranscript(text){return String(text||'').replace(/\r/g,'\n').split(/\n+|(?<=[.!?。！？])\s+/).map(x=>x.replace(/^[-•·\d.)\s]+/,'').replace(/\s+/g,' ').trim()).filter(x=>x.length>2)}
    function estimateCues(text,duration){const lines=splitTranscript(text);if(!lines.length)return[];const weights=lines.map(x=>Math.max(2,(x.match(/[A-Za-z]+(?:'[A-Za-z]+)?|[\u4e00-\u9fff]/g)||[]).length)),total=weights.reduce((a,b)=>a+b,0),span=Number.isFinite(duration)&&duration>10?Math.max(1,duration-2):Math.max(30,total/2.25);let used=0;return lines.map((line,index)=>{const time=used/total*span;used+=weights[index];return{index,time,label:formatCueTime(time),text:line,estimated:true}})}
    function renderSyncTranscript(){const field=$('#intensiveTranscript'),audio=$('#intensiveAudio');syncCues=parseSyncTranscript(field.value);let autoMode=false;if(!syncCues.length&&current?.record?.html){syncCues=extractStaticTranscript(current.record.html);if(!syncCues.length){const raw=extractTranscriptText(current.record.html);if(raw){syncCues=estimateCues(raw,audio.duration);autoMode=true}}if(syncCues.length&&!field.value.trim())field.value=syncCues.map(c=>'['+c.label+'] '+c.text).join('\n')}$('#intensiveSyncList').innerHTML=syncCues.length?(autoMode?'<p class="auto-transcript-note">已从题目文件自动提取原文；无原始时间戳时，点击时间按音频长度智能估算。</p>':'')+syncCues.map((cue,i)=>'<button data-sync-cue="'+i+'"><time>'+esc(cue.label)+'</time><span>'+esc(cue.text)+'</span></button>').join(''):'<p>这篇题目文件没有包含可读取的原文。可粘贴带时间的原文；仅靠浏览器无法准确把音频自动转写成文字。</p>';$('[data-sync-cue]').forEach(b=>b.onclick=()=>{const cue=syncCues[Number(b.dataset.syncCue)],player=$('#intensiveAudio');if(!cue||!player.src)return alert('当前设备没有这篇音频。');player.currentTime=cue.time;player.play()})}
    function highlightSyncCue(){if(!syncCues.length)return;const now=$('#intensiveAudio').currentTime;let active=0;syncCues.forEach((cue,i)=>{if(cue.time<=now)active=i});$('[data-sync-cue]').forEach((b,i)=>b.classList.toggle('active',i===active))}
