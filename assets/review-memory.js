(() => {
  ...
  function setupSpeakingMemory(){
    ...
    function renderCats(){
      ...
      $('[data-sm-filter]').forEach(b=>b.onclick=()=>{filter=b.dataset.smFilter;if(filter!=='全部')select.value=filter;studyIndex=0;render()});
      $$('[data-sm-part]').forEach(b=>b.classList.toggle('active',b.dataset.smPart===partView));updateAutoTarget();
    }
    ...
    $$('[data-sm-part]').forEach(b=>b.onclick=()=>{partView=b.dataset.smPart;filter='全部';studyIndex=0;render()});
