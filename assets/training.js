(() => {
  ...
  $('#addReadingCards').addEventListener('click',()=>{const existing=new Set(readingCards.map(x=>readingKey(x.front))),custom=JSON.parse(localStorage.getItem(readingCustomKey)||'[]');let added=0;for(const line of $('#readingImport').value.split('\n')){const p=line.split(/[|｜]/).map(x=>x.trim());if(p.length<2||!p[0]||!p[1]||existing.has(readingKey(p[0])))continue;const card={id:`RC-${Date.now()}-${added}`,front:p[0],meaning:p[1],example:p[2]||'',note:p[3]||'来自阅读生词积累'};custom.push(card);readingCards.push(card);existing.add(readingKey(p[0]));added++}localStorage.setItem(readingCustomKey,JSON.stringify(custom));$('#readingImport').value='';buildReadingQueue();alert(`已加入 ${added} 张阅读卡。`) });
