(() => {
  ...
  $('#saveWritingAI').onclick=()=>{if(!currentSession)return;const text=$('#writingAIImport').value.trim();if(!text)return alert('请先粘贴 ChatGPT 的批改报告。');const parsed=parseAIReport(text,currentSession),items=getHistory(),item=items.find(x=>x.id===currentSession.id);if(item)item.aiReport=text;currentSession.aiReport=text;saveHistory(items);let added=0;parsed.vocabulary.forEach(v=>{if(window.addIELTSWritingMemoryCard?.({kind:currentSession.task==='task1'?'task1':'task2',category:currentSession.topic||currentSession.theme||(currentSession.task==='task1'?'其他':'其他'),front:v.expression||v.front||'',back:v.meaning||v.back||'',example:v.example||''}))added++});renderReport(currentSession);alert(`ChatGPT 批改已自动归类并覆盖站内初评：逐句修改 ${parsed.sentenceCorrections.length} 条，表达 ${parsed.vocabulary.length} 条；其中 ${added} 条已加入作文专用记忆卡${parsed.revisedEssay?'，修改稿已归档':'。'}`)};
