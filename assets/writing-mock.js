(() => {
  ...
  function parseAIReport(text,session){
    ...
    if(!data)return {legacy:true,overall:findScore(['总分','overall(?: band)?']),taskScore:findScore([session.task==='task1'?'TA|Task Achievement':'TR|Task Response']),cc:findScore(['CC|Coherence(?: & | and )Cohesion']),lr:findScore(['LR|Lexical Resource']),gra:findScore(['GRA|Grammatical Range(?: & | and )Accuracy']),overallFeedback:'ChatGPT 完整批改已优先覆盖站内初评。此报告是旧版文本格式，原文完整保留在“逐句批改”中；下次使用新版复制提示可自动拆成逐句卡片。',sentenceCorrections:[],revisedEssay:'',vocabulary:[],nextSteps:[],raw:text};
  ...
  function renderAIReport(ai,session){
    ...
    $('#writingCorrections').innerHTML=ai.sentenceCorrections.length?ai.sentenceCorrections.map((x,i)=>`<article class="writing-sentence-correction"><span>${esc(x.type||'逐句修改')} · ${i+1}</span><div><del>${esc(x.original||'原句未提供')}</del><b>→</b><ins>${esc(x.revised||'修改句未提供')}</ins></div><p>${esc(x.reason||'ChatGPT 未单独说明原因')}</p></article>`).join(''):`<div class="writing-legacy-report"><p>这份回复没有可识别的逐句 JSON 卡片，以下仍以 ChatGPT 内容覆盖站内初评：</p><pre>${esc(ai.raw)}</pre></div>`;
