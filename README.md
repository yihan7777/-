# IELTS Speaking 2026 · 9 月 1 日冲刺系统

这套系统把 2026 年 5–8 月题库中的 **32 个 Part 1 主题、49 道 Part 2 题目及对应 Part 3 追问**，整理为人物、地点、事件/经历/计划、物品/法律/食物/规则 **4 大串题素材模块**，并按照 **D+1 / D+3 / D+7 / D+14** 安排复习。

## 每天怎么用

1. 打开可视化面板，看“今天只做这些”。
2. 点击 **复制今日 Context**，全文复制给 ChatGPT，然后开启 Live 语音。
3. 完成 Part 1 → Part 2 → Part 3 练习，等 ChatGPT 输出评分和反馈摘要。
4. 点击 **提交 Live 反馈**，把摘要填入 Issue；GitHub Actions 会自动更新 `data/feedback.json`、`data/progress.json` 和下一轮 Context。
5. 在 **词汇记忆卡片** 中先回忆再翻面，根据真实掌握程度选择“忘了 / 困难 / 掌握 / 简单”；手机会保存复习进度并自动安排下次出现时间。
6. 在 **作文打字训练** 中粘贴范文并完成复刻，查看 WPM、准确率、拼写/漏词/多词报告和自动提取的表达。
7. 在 **听力单词反应卡** 中只听单词与例句，不看拼写；先写答案，再翻面核对并安排复习。
8. 在 **阅读生词与表达卡** 中批量加入阅读材料里的单词、搭配和原句，按语境翻面记忆并自动复习。

## 自动化

- 每天北京时间 08:00 生成 `daily-context/latest.md`。
- 新建带 `practice-feedback` 标签的反馈 Issue 后，自动同步练习记录。
- `main` 分支更新后自动部署 GitHub Pages。
- 内置 36 张个性化表达卡片，覆盖旅行、固定句型、Part 3 逻辑、学习成长和重点发音。
- 每次 Live 练习从反馈中提取 10 个表达，自动去重并加入“每日练习”卡组。
- 作文打字记录、累计错词和自定义听力卡保存在当前手机浏览器中。
- 每次作文打字完成后会按作文题目保存范文与历史成绩，可展开查看并点击“再次练习”。
- 自定义阅读卡同样保存在当前手机浏览器中，可一键复制导出备份。
- Part 2 素材库包含 78 条贴合个人素材的 6.5 分表达和情节句，并同步进入“口语串题”记忆卡组。

## 计划原则

- 8 月 11–18 日：每天建立 1 个故事骨架，覆盖该组所有 Part 2 题目。
- 8 月 12 日起：按照 D+1、D+3、D+7、D+14 脱稿复现。
- 8 月 23–31 日：进入完整模考与薄弱项回炉。
- 9 月 1 日：只做轻量热身，不再增加新内容。

数据来自用户提供的《2026 年 5–8 月雅思口语神奇题库第三版（视频口语）》。本仓库用于个人备考。
